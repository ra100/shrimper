...
| R4.7 | Commit Phase R4. Message: `✨ feat: wire condition + achievements into app lifecycle`. | git log shows commit | R4.1, R4.2, R4.3, R4.4, R4.5, R4.6 | cc:完了 [c97ec2b] |

## Phase R5: Cleanup

| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| R5.1 | Delete all PNG assets: `git rm public/characters/stage{1,2,3,4,5}-{happy,neutral,sad}.png` (15 files). | `ls public/characters/` empty or gone | Phase R2 | cc:完了 [c97ec2b] |
| R5.2 | Delete `git rm scripts/generate-shrimps.py`. If `scripts/` is empty, `git rm -r scripts/` (or leave). | File gone | R5.1 | cc:完了 [c97ec2b] |
| R5.3 | Delete `git rm docs/art-prompts.md`. | File gone | R5.1 | cc:完了 [c97ec2b] |
| R5.4 | Delete `git rm flux2_dev_workflow.json`. | File gone | R5.1 | cc:完了 [c97ec2b] |
| R5.5 | Commit Phase R5. Message: `🔥 chore: remove abandoned Flux/ComfyUI PNG art pipeline`. | Working tree clean for deleted files | R5.1, R5.2, R5.3, R5.4 | cc:完了 [c97ec2b] |

## Phase R6: Validation

| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| R6.1 | Run `npm run build`. | Exit 0; `dist/` produced; zero TS errors | Phase R4, Phase R5 | cc:完了 [c97ec2b] |
